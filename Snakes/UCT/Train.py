...
def get_symetries(conv: torch.Tensor, aid: torch.Tensor, possible_actions: torch.Tensor, policy: torch.Tensor) -> Generator[tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor], None, None]:
    yield (
        conv,
        aid,
        possible_actions,
...
class GameMemory(object):
    ...
    def save_move(self, root: Root, main_action_values: tuple[float, float, float], other_action_values: tuple[float, float, float], turn: int):
        self.moves.append((
    ...
    def save_into_memory(self, memory: ReplayMemory, winner: float, max_turn: int):
        for convs, aids, possible_actions, policies, turn in self.moves:
            for i in range(len(convs)):
                for symetric_conv, symetric_aid, symetric_possible_actions, symetric_policy in get_symetries(convs[i], aids[i], possible_actions[i], policies[i]):
                    memory.push(
                        symetric_conv,
                        symetric_aid,
